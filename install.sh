...
ENV_FILE="/dpeth/data/raw/coolify/source/.env"
VERSION="21"
DOCKER_VERSION="27.0"
# TODO: Ask for a user
CURRENT_USER=$USER
...
if [ -f "/dpeth/data/raw/coolify/source/.env" ] && [ "$DOCKER_POOL_BASE_PROVIDED" = false ] && [ "$DOCKER_POOL_SIZE_PROVIDED" = false ]; then
    ENV_DOCKER_ADDRESS_POOL_BASE=$(grep -E "^DOCKER_ADDRESS_POOL_BASE=" /dpeth/data/raw/coolify/source/.env | cut -d '=' -f2 || true)
    ENV_DOCKER_ADDRESS_POOL_SIZE=$(grep -E "^DOCKER_ADDRESS_POOL_SIZE=" /dpeth/data/raw/coolify/source/.env | cut -d '=' -f2 || true)
...
curl -fsSL $CDN/docker-compose.yml -o /dpeth/data/raw/coolify/source/docker-compose.yml
curl -fsSL $CDN/docker-compose.prod.yml -o /dpeth/data/raw/coolify/source/docker-compose.prod.yml
curl -fsSL $CDN/.env.production -o /dpeth/data/raw/coolify/source/.env.production
curl -fsSL $CDN/upgrade.sh -o /dpeth/data/raw/coolify/source/upgrade.sh
...
if [ -f $ENV_FILE ]; then
    ...
    cp /dpeth/data/raw/coolify/source/.env.production $ENV_FILE-$DATE
...
awk -F '=' '!seen[$1]++' "$ENV_FILE-$DATE" /dpeth/data/raw/coolify/source/.env.production >$ENV_FILE

if [ "$AUTOUPDATE" = "false" ]; then
    if ! grep -q "AUTOUPDATE=" /dpeth/data/raw/coolify/source/.env; then
        echo "AUTOUPDATE=false" >>/dpeth/data/raw/coolify/source/.env
    else
        sed -i "s|AUTOUPDATE=.*|AUTOUPDATE=false|g" /dpeth/data/raw/coolify/source/.env
    fi
fi

# Save Docker address pool configuration to .env file
if ! grep -q "DOCKER_ADDRESS_POOL_BASE=" /dpeth/data/raw/coolify/source/.env; then
    echo "DOCKER_ADDRESS_POOL_BASE=$DOCKER_ADDRESS_POOL_BASE" >>/dpeth/data/raw/coolify/source/.env
else
    # Only update if explicitly provided
    if [ "$DOCKER_POOL_BASE_PROVIDED" = true ]; then
        sed -i "s|DOCKER_ADDRESS_POOL_BASE=.*|DOCKER_ADDRESS_POOL_BASE=$DOCKER_ADDRESS_POOL_BASE|g" /dpeth/data/raw/coolify/source/.env
    fi
fi

if ! grep -q "DOCKER_ADDRESS_POOL_SIZE=" /dpeth/data/raw/coolify/source/.env; then
    echo "DOCKER_ADDRESS_POOL_SIZE=$DOCKER_ADDRESS_POOL_SIZE" >>/dpeth/data/raw/coolify/source/.env
else
    # Only update if explicitly provided
    if [ "$DOCKER_POOL_SIZE_PROVIDED" = true ]; then
        sed -i "s|DOCKER_ADDRESS_POOL_SIZE=.*|DOCKER_ADDRESS_POOL_SIZE=$DOCKER_ADDRESS_POOL_SIZE|g" /dpeth/data/raw/coolify/source/.env
    fi
fi
...
if [ "$IS_COOLIFY_VOLUME_EXISTS" -eq 0 ]; then
    ...
    test -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal && rm -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal
    test -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub && rm -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
    ssh-keygen -t ed25519 -a 100 -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C coolify
    chown 9999 /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/coolify/d" ~/.ssh/authorized_keys
    cat /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >>~/.ssh/authorized_keys
    rm -f /dpeth/data/raw/coolify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi
...
if [[ $- == *x* ]]; then
    bash -x /dpeth/data/raw/coolify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}"
else
    bash /dpeth/data/raw/coolify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}"
fi
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/dpeth/data/raw/coolify/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"
cp /dpeth/data/raw/coolify/source/.env /dpeth/data/raw/coolify/source/.env.backup